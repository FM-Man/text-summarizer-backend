...
def _similarity(sentence_x, sentence_y, sigma):
    ...
    #     ws_sum += exp(-(most_similar_distances[i] ** 2) / (2 * sigma ** 2))
    # ws_sum /= length
    #
    # # gaussian similarity
    # return ws_sum
    distance_matrix=np.zeros((len(sentence_x), len(sentence_y)))
    for i, vec1 in enumerate(sentence_x):
        for j, vec2 in enumerate(sentence_y):
            distance_matrix[i,j] = np.linalg.norm(vec1-vec2)

    p = np.ones(len(sentence_x))/len(sentence_x)
    q = np.ones(len(sentence_y))/len(sentence_y)
