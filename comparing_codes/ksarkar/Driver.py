...
def dataset_testing(document, summary_1, summary_2, sig, serial_no, result_compilation, rouge):
    print("#######",sig,"->",serial_no,"#############")
    row = {}
...
for sig in sigmas:
    for row_index, row in documents_summaries_1.iterrows():
        # try:
        dataset_testing(row["Document"],row["summary_1"],row["summary_2"],sig,serial_no,result_compilation,rouge)
        serial_no+=2
        # except: pass
...
resultComp.close()
json.dump(result_compilation, open("KsarkarResult_dataset_4_" + str(sig) + ".json", "w", encoding="utf-8"), ensure_ascii=False, indent=4)
