...
def _spectral_clustering(set_of_vectors, sigma, size):
    ...
    if len(affinity_matrix) > 1:
        ...
        return aff_copy, cluster_indices
...
def get_summary(text, sigma=2, size=.1):
    ...
    affinity, set_of_clusters = _spectral_clustering(set_of_vectors, sigma, size)  # step 4
    ...
    for cluster in set_of_clusters.items():
        ...
        #aff_total
        list_of_total_affinity = []
        for i in range(len(cluster[1])):
            total_affinity_for_the_sentence = 0
            for j in range(len(cluster[1])):
                # print("affinity between sentence {} and sentence {} is {}".format(cluster[1][i], cluster[1][j], affinity[cluster[1][i]][cluster[1][j]]))
                total_affinity_for_the_sentence += affinity[cluster[1][i]][cluster[1][j]]

            list_of_total_affinity.append(total_affinity_for_the_sentence)

        # print(list_of_total_affinity)
        max_index = list_of_total_affinity.index(max(list_of_total_affinity))
        print(max_index)
        picked_index = cluster[1][max_index]
